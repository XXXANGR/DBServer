...
extern int8_t listen_http(HTTP *http)
{
	...
	while (1)
	{
		...
		else if (pid == 0)
		{
			...
			_switch_http(http, conn, &req); /* Обрабатываем запрос */
			close_net(conn);
			exit(0);
		}
		else
...
extern void parse_http(int conn, char *filename, type_http_t type, char *tbl_name)
{
	...
	if (type == HTTP_TEXT || type == HTTP_TEXT_TABLE)
	{
		strcpy(buffer, "HTTP/1.1 200 OK\nContent-type: text/html\n\n");
	}
	else if (type == HTTP_IMG_JPG)
	{
	...
	while ((readsize = fread(buffer, sizeof(char), BUFSIZ, file)) != 0) {
		// Ищем тег <table ... > и вставляем после него ответ от БД
		char *table_ptr;
		if ((type == HTTP_TEXT_TABLE) && (table_ptr = strstr(buffer, "<table \"price\"></table>")))
		{
			size_t len = table_ptr - buffer;
			send_net(conn, buffer, len - 20);
			
			FILE *tbl_file = fopen("Table.html", "w+");
			if (tbl_file == NULL) {
				printf("Can't create file for table\n");
				return;
			}

			get_table(tbl_file, tbl_name);

			size_t tbl_readsize;
			char tbl_buffer[BUFSIZ];
			while ((tbl_readsize = fread(tbl_buffer, sizeof(char), BUFSIZ, tbl_file)) != 0) {
				send_net(conn, tbl_buffer, tbl_readsize);
			}
			
			fclose(tbl_file);
			if (remove("Table.html") == -1) {
				printf("Error removing table file\n");
			}

			send_net(conn, buffer + len + 24, readsize - len - 24);
		}
		else
		{
			send_net(conn, buffer, readsize);
...
static int8_t _switch_http(HTTP *http, int conn, HTTPreq *request)
{
	if (!in_hashtab(http->tab, string(request->path)))
	{
		...
		http->funcs[index](conn, request); /* Отправляем страничку */
		return 0;
	}
